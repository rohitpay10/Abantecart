...
class ControllerResponsesExtensionBpay extends AController {
    ...
    $template_data['button_confirm'] = $this->language->get('button_confirm');
     ...
     // echo $template_data['ORDER_ID'];exit;
      ...
      // print_r($template_data);exit;
	    ...
	    //Request Url
			  ...
			  $pg_transaction->setPgRequestUrl($request_url);
				...
				$pg_transaction->setReturnUrl($template_data['CALLBACK_URL']);
	  ...
	  public function callback() {
		   // print_r($_POST);
			 $template_data['BPAY_MERCHANT_KEY'] = $this->config->get('bpay_merchant_key');
			 $template_data['BPAY_MERCHANT_HOSTED_KEY']= $this->config->get('bpay_merchant_hosted_key');
		   $salt = $template_data['BPAY_MERCHANT_KEY'];
		   $Merchant_hosted_key = $template_data['BPAY_MERCHANT_HOSTED_KEY'];
			 $var = new BPPGModule;
	     $var->setSalt($salt);
	     $var->setMarchantHostedKey($Merchant_hosted_key);
	     $data = $var->handleResponse($_POST['ENCDATA']);
	     $all_Data = $var->split_decrypt_string($data); 
       //echo "<pre>"; var_dump($all_Data);"</pre>";die();
	     ...
	     if($all_Data['STATUS'] == 'Captured' && $all_Data['RESPONSE_CODE'] == '000' && 
	      $all_Data['RESPONSE_MESSAGE'] == 'Successful Payment'){
	    $this->load->model('checkout/order');

	     // $order_id=$all_Data['ORDER_ID'];

	     $this->model_checkout_order->confirm($order_id ,$this->config->get('bpay_order_status_id'));
